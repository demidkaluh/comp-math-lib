...
// TODO: Матрица веса B=diag{lambda_i}, A^t * B * A * x = A^t * B * f

Matrix weightMatrix (int n)
{
  Matrix res (n);
  for (size_t i = 0; i < n; i++)
    for (size_t j = 0; j < n; j++)
...
std::vector<double>
solve_overdetermined_SLE_with_weightMatrix (Matrix m, std::vector<double> f,
                                            Matrix w)
{
  if (w.getRowLen () != m.getRowLen ())
  {
    printf ("Error : weight matrix dimension must be as in overdetermined "
            "matrix row size\n");
    exit (-1);
  }
  ...
  if (!w.isSquare ())
  {
    printf ("Error : weight matrix must be square\n");
    exit (-1);
  }
  return solve_SLE_Jacobi (transpose (m) * w * m, transpose (m) * w * f);
}
